...
// Tabs that fetch data from the API
const API_TABS: Partial<Record<Tab, string>> = {
  ...
  roles: '/admin/students', // reuse students for role management
};

// Tabs that are local-only (no API fetch needed)
const LOCAL_TABS: Tab[] = ['announcements', 'content'];
...
useEffect(() => {
  ...
  useEffect(() => {
    // Only fetch if this tab requires an API call
    if (!LOCAL_TABS.includes(activeTab)) {
      fetchTab(activeTab);
    } else {
      setLoading(false);
    }
  ...
  const handleAddRole = async (userId: string, role: string) => {
    ...
    try {
      await api.post(`/roles/${userId}/add-role`, { role });
      fetchTab('roles');
    } catch (err: any) {
  ...
  const handleGrantModule = async (userId: string, module: string) => {
    setRoleActionLoading(true);
    try {
      await api.post(`/roles/${userId}/grant-module`, { module });
      alert(`Module ${module} granted.`);
    } catch (err: any) {
      alert(err?.response?.data?.message || 'Failed to grant module');
    } finally {
      setRoleActionLoading(false);
    }
  };
      ...
      ">
        <div>
          <div className="hidden md:block mb-6">
            ...
            <p className="text-white/20 text-xs pl-8 truncate">{user?.email}</p>
          </div>
          ...
          <nav className="flex md:flex-col gap-1 overflow-x-auto md:overflow-visible pb-1 md:pb-0">
            {tabs.map((tab) => (
              <button
                ...
                className={`flex items-center gap-2 shrink-0 md:w-full px-3 py-2.5 text-sm transition rounded-sm whitespace-nowrap ${
                  activeTab === tab.key
        ...
        <div className="hidden md:block mt-4">
          <Link
            href="/"
            className="flex items-center gap-2 text-white/20 hover:text-white text-xs transition px-3 py-2 mb-1"
          >
            ← View Site
          </Link>
          <button
            onClick={handleLogout}
            className="flex items-center gap-2 text-white/20 hover:text-white text-sm transition px-3 py-2 w-full"
          >
      ...
      <main className="flex-1 overflow-auto min-w-0">
        ...
        <div className="border-b border-white/5 px-4 md:px-8 py-4 flex items-center justify-between sticky top-0 bg-black z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={handleLogout}
              className="flex items-center gap-1.5 text-white/20 hover:text-white text-xs transition md:hidden"
            >
              <LogOut size={13} />
            </button>
          </div>
        ...
        <div className="p-4 md:p-8">
          {loading && !LOCAL_TABS.includes(activeTab) ? (
            <div className="flex items-center justify-center py-24">
              <div className="text-center">
                <div className="w-6 h-6 border border-brand-blue border-t-transparent rounded-full animate-spin mx-auto mb-3" />
            ...
            <>
              ...
              {activeTab === 'announcements' && (
                <div className="max-w-2xl space-y-6">
                  <div>
                    <h2 className="font-bebas text-2xl text-white mb-1">CREATE ANNOUNCEMENT</h2>
                    <p className="text-white/30 text-sm mb-5">
                      Announcements appear on the homepage and community page automatically.
                    </p>
                    <form onSubmit={handleCreateAnnouncement} className="space-y-4">
                      <div>
                        ...
                        <input
                          ...
                          placeholder="e.g. New cohort open for enrollment"
                        />
                      </div>
                      <div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="flex items-center gap-3 pt-6">
                          ...
                          />
                          <label htmlFor="pinned" className="text-white/50 text-sm cursor-pointer">
                            Pin to top
                          </label>
                        </div>
                      </div>
              ...
              {activeTab === 'content' && (
                <div className="max-w-2xl space-y-8">
                  <div>
                    <h2 className="font-bebas text-2xl text-white mb-1">ADD TESTIMONIAL</h2>
                    <p className="text-white/30 text-sm mb-5">
                      Testimonials appear on the homepage automatically.
                    </p>
                    <form onSubmit={handleCreateTestimonial} className="space-y-4">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setTestimonialForm({
                                ...testimonialForm,
                                name: e.target.value,
                                avatar_initial: e.target.value[0] || '',
                              })
                            }
                            className={inputClass}
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          className={`${inputClass} resize-none`}
                          placeholder="What they said about TechMindsVerse..."
                        />
                      </div>
                      <button
                        type="submit"
                        ...
                        <div className="flex items-start justify-between flex-wrap gap-4">
                          <div>
                            ...
                            {payment.proof_image_url && (
                               ...
                               <a href={payment.proof_image_url}
                                target="_blank"
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            <span
                              className={`text-xs border px-3 py-1 capitalize ${statusColor(payment.status)}`}
                            >
                              {payment.status}
                            </span>
                        ...
                        <div className="flex items-center justify-between flex-wrap gap-4">
                          <div>
                            ...
                            <p className="text-white/20 text-xs mt-1 capitalize">
                              Track: {student.track || 'Not assigned'} · Score:{' '}
                              {student.performance_score ?? '—'}
                            </p>
                          </div>
                          <span
                            className={`text-xs border px-3 py-1 capitalize ${statusColor(student.users?.status)}`}
                          >
                    ...
                    data.map((project: any) => (
                      <div key={project.id} className="border border-white/5 p-5 hover:border-white/10 transition">
                        <div className="flex items-start justify-between flex-wrap gap-4">
                          ...
                          <span
                            className={`text-xs border px-3 py-1 capitalize ${statusColor(project.status)}`}
                          >
                            {project.status}
                          </span>
                        </div>
                    ...
                    data.map((complaint: any) => (
                      <div key={complaint.id} className="border border-white/5 p-5 hover:border-white/10 transition">
                        <div className="flex items-start justify-between flex-wrap gap-4">
                          ...
                          <span
                            className={`text-xs border px-3 py-1 capitalize ${statusColor(complaint.status)}`}
                          >
                            {complaint.status}
                          </span>
                        </div>
                      ...
                      <div key={build.id} className="border border-white/5 p-5 hover:border-white/10 transition">
                        <div className="flex items-start justify-between flex-wrap gap-4">
                          ...
                          <div className="flex flex-col items-end gap-2 shrink-0">
                            <span
                              className={`text-xs border px-3 py-1 capitalize ${statusColor(build.status)}`}
                            >
                              {build.status}
                            </span>
                            ...
                            <div className="flex flex-wrap gap-1 justify-end">
                              ...
                              {build.status === 'in_progress' && (
                                <>
                                  <select
                                    onChange={(e) =>
                                      handleProgressUpdate(build.id, Number(e.target.value))
                                    }
                    ...
                    <div className="border border-white/5 p-6">
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span
                            className={`text-xs border px-2 py-0.5 capitalize ${statusColor(selectedBuild.status)}`}
                          >
                ...
                <div className="space-y-2">
                  {data.length === 0 ? (
                    ...
                    data.map((activity: any) => (
                      <div
                        key={activity.id}
                        className="border border-white/5 px-5 py-3 flex items-center justify-between"
                      >
                        <div className="flex items-center gap-3">
              ...
              {activeTab === 'roles' && (
                <div className="space-y-6">
                  <div>
                    <h2 className="font-bebas text-2xl text-white mb-1">ROLE MANAGEMENT</h2>
                    <p className="text-white/30 text-sm mb-5">
                      Assign roles and module access to users.
                    </p>
                  </div>
                  {data.length === 0 ? (
                        ...
                        <div key={student.id} className="border border-white/5 p-5 hover:border-white/10 transition">
                          <div className="flex items-start justify-between flex-wrap gap-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {['student', 'client', 'builder', 'instructor'].map((role) => (
                                <button
                                  key={role}
                                  onClick={() => handleAddRole(student.users?.id, role)}
                                  disabled={
                                    roleActionLoading ||
                                    student.users?.role === role
                                  }
                                  className="text-xs border border-white/10 text-white/40 px-2.5 py-1 hover:border-brand-blue/30 hover:text-brand-blue transition disabled:opacity-30 capitalize"
                                >
                                  +{role}
                                </button>
                              ))}
                              {['academy', 'build_studio', 'community'].map((module) => (
                                <button
                                  key={module}
                                  onClick={() => handleGrantModule(student.users?.id, module)}
                                  disabled={roleActionLoading}
                                  className="text-xs border border-purple-500/20 text-purple-400/60 px-2.5 py-1 hover:border-purple-500/40 hover:text-purple-400 transition disabled:opacity-30 capitalize"
                                >
                                  +{module.replace('_', ' ')}
                                </button>
                              ))}
